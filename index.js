...
async function run() {
  try {
    ...
    app.patch("/notifications/mark-read/:id", verifyFBToken, async (req, res) => {
      try {
    ...
    app.get("/notifications/unread-count/:email", verifyFBToken, async (req, res) => {
      try {
        const email = req.params.email;
    ...
    // admin
    app.patch("/users/coins/admin/:email",verifyFBToken, verifyAdmin, async (req, res) => {
      const email = req.params.email;
      const { refundAmount } = req.body;
      await usersCollection.updateOne(
        { email },
    ...
    app.get("/approved-tasks", verifyFBToken, verifyWorker, async (req, res) => {
      try {
    ...
    app.get("/worker-submissions", verifyFBToken, verifyWorker, async (req, res) => {
      try {
        const { workerEmail, page = 1, limit = 10 } = req.query;
        const skip = (page - 1) * limit;
    ...
    app.post("/task-submissions", verifyFBToken, verifyWorker, async (req, res) => {
      try {
    ...
    // Admin related APIs
    app.delete("/tasks/admin/:id", verifyFBToken, verifyAdmin, async (req, res) => {
      const taskId = req.params.id;
      const result = await taskCollection.deleteOne({
        _id: new ObjectId(taskId),
      });
    ...
    // admin
    app.get("/pending-submissions/admin", verifyFBToken, verifyAdmin, async (req, res) => {
      try {
        const buyerEmail = req.query.buyerEmail;
        const submissions = await taskSubmissionsCollection
          .find({
    ...
    app.patch("/approve-submission/:id", verifyFBToken, verifyBuyer, async (req, res) => {
      try {
    ...
    app.patch("/reject-submission/:id", verifyFBToken, verifyBuyer, async (req, res) => {
      try {
        const submissionId = req.params.id;
    ...
    app.get("/withdrawal-requests", verifyFBToken, verifyAdmin, async (req, res) => {
      try {
    ...
    app.patch("/approve-withdrawal/:id", verifyFBToken, verifyAdmin, async (req, res) => {
      try {
        const { id } = req.params;
    ...
    app.get("/tasks/states/buyer/:email", verifyFBToken, verifyBuyer, async (req, res) => {
      const email = req.params.email;
      const count = await taskCollection.countDocuments({ created_by: email });
    ...
    app.get("/pendingTasksCount/admin", verifyFBToken, verifyAdmin, async (req, res) => {
      const email = req.query.email;
      const count = await taskSubmissionsCollection.countDocuments({
        Buyer_email: email,
        status: "pending",
    ...
    app.get("/tasks/states/worker/:email", verifyFBToken, verifyWorker, async (req, res) => {
      const email = req.params.email;
